...
public class Exam implements Serializable {
   ...
   /* public static void main(String[] args) {
        ...
        try {
            ...
            for (Exam exam:Exam.findAll())
                {
                    if(exam.getExamDate()!=null)
                        System.out.println(exam.getExamDate().toLocalDate()+" "+exam.getExamDate().toLocalTime() );}
    ...
    public Exam(JSONObject json) throws JSONException
    {
        ...
        {      
            System.out.println(json.getString(examDateTemplate));
    ...
    public List<Answer> findAnswersbyExam() throws JSONException, IOException
    {
        URL url = new URL("http://localhost:8080/ServerApp/webresources/data.answer/findbyexam/"+this.examId);
        String request=GETrequest(url);
        JSONArray arr=new JSONArray(request);
    ...
    public List<Examinee> findExamineessbyExam() throws JSONException, IOException
    {
        URL url = new URL("http://localhost:8080/ServerApp/webresources/data.examinee/findbyexam/"+this.examId);
        String request=GETrequest(url);
        JSONArray arr=new JSONArray(request);
        ...
        if (arr!=JSONObject.NULL) 
        {
            ...
            { // Walk through the Array.
                JSONObject obj = arr.getJSONObject(i);
                Examinee temp=new Examinee(obj);
                examinees.add(temp);
            }       
            return examinees;
        }
